  ...
  >
    <div @click="openUser" class=" flex gap-2 justify-between px-2">
      ...
      <div class="flex flex-col justify-center h-full  items-end w-1/5 ">
        <span
          class="text-[16px] flex items-center gap-2 font-semibold text-main"
        >
          <span class="text-sm font-normal text-[#D1D1D1]"
            >{{ summUser2.toLocaleString('ru-RU') }}
          </span>
          +{{ summUser.toLocaleString('ru-RU') }}₽</span
        >
        <span class="text-[12px] text-[#343434]/70 font-normal"
          >{{ user_data.attributes.zakazies.data.length }} заказа</span
        >
      </div>
    </div>
      ...
      <div v-if="active == user_data.id && type == 'dostupno'" class="anime ">
        <div class="grid grid-cols-1 ">
          <div
            ...
            :key="i"
            class="flex justify-between items-center border-b border-[#343434]/30 px-2 py-2"
          >
            <div class="flex flex-col items-start justify-center">
              <span class="text-[16px] font-semibold">№ {{ order.id }} </span>
           ...
           <nuxt-link :to="`/doctor/order/` + order.id" class="text-sm">Подробнее</nuxt-link>
            <span
              ...
              >+{{
                ((order.attributes.SummOrder / 100) * 20).toLocaleString(
                  'ru-RU'
                )
              }}₽</span
            >
            <span v-else class="text-[12px]  font-semibold"
